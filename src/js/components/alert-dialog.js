// src/ts/components/alert-dialog.ts
class AlertDialogAction extends HTMLElement {
  _styles = `rounded-md px-4 py-[7px] bg-black dark:bg-white text-white dark:text-black flex
...
class AlertDialogCancel extends HTMLElement {
  _styles = `rounded-md px-4 py-[7px] bg-white dark:bg-black text-black dark:text-white
...
class AlertDialogFooter extends HTMLElement {
  _style = `flex justify-end w-full gap-4 text-sm`;
  constructor() {
    super();
...
class AlertDialogDescription extends HTMLElement {
  _style = `text-sm font-normal text-black/55 dark:text-white/55`;
  constructor() {
    super();
    this.className = `${this._style} ${this.className}`;
  }
...
class AlertDialogContent extends HTMLElement {
  _style = `flex flex-col gap-6 w-lg bg-white dark:bg-black ring-1 
...
class AlertDialog extends HTMLElement {
  ...
  constructor() {
    super();
    const triggerEl = this.querySelector("alert-dialog-trigger");
    const contentEl = this.querySelector("alert-dialog-content");
    if (triggerEl == null || contentEl == null || triggerEl.firstElementChild == null) {
      console.error("Structural error in alert dialog");
      return;
    }
    const cancelEl = contentEl.querySelector("alert-dialog-cancel");
    if (cancelEl == null) {
      console.error("Structural error in alert dialog");
      return;
    }
    this._trigger = triggerEl.firstElementChild;
  ...
  _openDialog() {
    ...
    if (this._content) {
      ...
      let content = this._content.cloneNode(true);
